...
object Day2 extends App with CodeGolf {
  ...
  val part1 = input
    .map(SledRentalPassword.apply)
    .count(_.isValid)
  val part2 = input
    .map(TobogganCorpPassword.apply)
    .count(_.isValid)
...
object SledRentalPassword {
  def apply(input: String): SledRentalPassword = {
    input match {
      case Password.Matcher(min, max, letter, password) =>
        SledRentalPassword(letter.charAt(0), min.toInt, max.toInt, password)
      case _ =>
        throw new IllegalArgumentException(
          s"""Invalid formatting of password line: "$input""""
        )
    }
  }
}

case class TobogganCorpPassword(
    letter: Char,
    pos1: Int,
    pos2: Int,
    password: Array[Char]
